...
class ChatViewModel(private val repo: ChatRepository) : ViewModel() {
    data class UiState(val messages: List<Message> = emptyList(), val isStreaming: Boolean = false, val error: String? = null)
    ...
    init {
        viewModelScope.launch {
            repo.messages.collect { _uiState.update { it.copy(messages = messages) } }
        }
    }
    ...
    fun sendMessage(input: String) {
        ...
        viewModelScope.launch {
            runCatching { repo.sendMessage(input).collect {} }
                .onFailure { _uiState.update { it.copy(error = "Error: ${it.message}") } }
